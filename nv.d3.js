...
/*
This function was created to allow for two-color scatter2Y
*/
nv.utils.getColor2 = function (color) {
    //if you pass in nothing, get default colors back
    if (color === undefined) {
        return nv.utils.defaultColor();
        ...
        return function (d, i) {
            var key = i === undefined ? d : i;
            return d.color2 || color_scale(key);
        };
...
//Added for scatter2Y separate distributions
nv.models.distribution2 = function () {
    "use strict";
    //============================================================
    // Public Variables with Default Settings
    //------------------------------------------------------------
...
//Scatter plot with 2 y-axes
nv.models.scatter2Y = function() {
    "use strict";
    ...
    var margin       = {top: 0, right: 0, bottom: 0, left: 0},
        ...
        , getX         = function(d) { return d.x } // accessor to get the x value
        , getY = function (d) { return d.y } // accessor to get the y value
        , getY2 = function (d) { return d.y2 } //accessor to get the y2 value
        , getSize      = function(d) { return d.size || 1} // accessor to get the point size
        , getShape     = function(d) { return d.shape || 'circle' } // accessor to get point shape
        , forceX       = [] // List of numbers to Force into the X scale (ie. 0, or a max / min, etc.)
        , forceY       = [] // List of numbers to Force into the Y scale
    ...
    var x0, y0, y20, z0 // used to store previous scales
        , timeoutID
        , needsUpdate = false // Flag for when the points are visually updating, but the interactive layer is behind, to disable tooltips
        , renderWatch = nv.utils.renderWatch(dispatch, duration)
        , _sizeRange_def = [16, 256]
    ...
    function chart(selection) {
        renderWatch.reset();
        var totMargin = { top: Math.max(margin.top, margin2.top), bottom: Math.max(margin.bottom, margin2.bottom), left: Math.max(margin.left, margin2.left), right: Math.max(margin.right, margin.left) }
        selection.each(function(data) {
            container = d3.select(this);
            var availableWidth = nv.utils.availableWidth(width, container, margin),
                availableHeight = nv.utils.availableHeight(height, container, margin);
            ...
            var seriesData = (xDomain && yDomain && sizeDomain) ? [] : // if we know xDomain and yDomain and sizeDomain, no need to calculate.... if Size is constant remember to set sizeDomain to speed up performance
                d3.merge(
                    data.map(function(d) {
                        return d.values.map(function(d,i) {
                            return { x: getX(d,i), y: getY(d,i), y2:getY2(d,i), size: getSize(d,i) }
                        })
                    })
                );
            ...
            //x.range([availableWidth * .5 / data[0].values.length, availableWidth * (data[0].values.length - .5)  / data[0].values.length ]);
             ...
             if (logScale) {
                    ...
                    y2.clamp(true)
                        .domain(y2Domain || d3.extent(seriesData.map(function (d) {
                            if (d.y2 !== 0) return d.y2;
                            else return min * 0.1;
                        }).concat(forceY)))
                        .range(yRange || [availableHeight, 0]);
                    } else {
                        y.domain(yDomain || d3.extent(seriesData.map(function (d) { return d.y;}).concat(forceY)))
                        .range(yRange || [availableHeight, 0]);
                        y2.domain(y2Domain || d3.extent(seriesData.map(function (d) { return d.y2; }).concat(forceY)))
                               .range(yRange || [availableHeight, 0]);
                }
            ...
            //if there is a second y-axis, create points:
            if (showY2Axis == true) {

                groups = wrap.select('.nv-groups').selectAll('.nv-group2')
                .data(function (d) { return d }, function (d) { return d.key });
                groups.enter().append('g')
                    .style('stroke-opacity', 1e-6)
                    .style('fill-opacity', 1e-6);
                ...
                groups
                    .attr('class', function (d, i) {
                        return (d.classed || '') + ' nv-group nv-series2-' + i;
                    })
                    .classed('nv-noninteractive', !interactive)
                    .classed('hover', function (d) { return d.hover });
                groups.watchTransition(renderWatch, 'scatter: groups')
                    ...
                    .style('stroke', function (d, i) { return color2(d, i) })
                    .style('stroke-opacity', 1)
                    .style('fill-opacity', .5);

                points = groups.selectAll('path.nv-point2')
                                .data(function (d) {
                                    return d.values.map(
                                        function (point, pointIndex) {
                                            return [point, pointIndex]
                ...
                points.enter().append('path')
                    ...
                    .attr('transform', function (d) {
                        return 'translate(' + nv.utils.NaNtoZero(x0(getX(d[0], d[1]))) + ',' + nv.utils.NaNtoZero(y20(getY2(d[0], d[1]))) + ')'
                    })
                    .attr('d',
                        nv.utils.symbol()
                        .type(function (d) { return getShape(d[0]); })
                ...
                points.exit().remove();
                groups.exit().selectAll('path.nv-point2')
                    .watchTransition(renderWatch, 'scatter exit')
                    .attr('transform', function (d) {
                        return 'translate(' + nv.utils.NaNtoZero(x(getX(d[0], d[1]))) + ',' + nv.utils.NaNtoZero(y2(getY2(d[0], d[1]))) + ')'
                    })
                    .remove();
                points.filter(function (d) { return scaleDiff || getDiffs(d, 'x', 'y2'); })
                    .watchTransition(renderWatch, 'scatter points')
                    .attr('transform', function (d) {
                        //nv.log(d, getX(d[0],d[1]), x(getX(d[0],d[1])));
                        return 'translate(' + nv.utils.NaNtoZero(x(getX(d[0], d[1]))) + ',' + nv.utils.NaNtoZero(y2(getY2(d[0], d[1]))) + ')'
    ...
    chart._calls = new function() {
        this.clearHighlights = function () {
            nv.dom.write(function() {
                container.selectAll(".nv-point.hover").classed("hover", false);
                container.selectAll(".nv-point2.hover").classed("hover", false);
            });
            return null;
        };
        this.highlightPoint = function (seriesIndex, pointIndex, isHoverOver) {
            nv.dom.write(function () {
                //-/Uncomment this to allow tooltip:
                //container.select('.nv-groups')
                //  .selectAll(".nv-series-" + seriesIndex)
                //  .selectAll(".nv-point-" + pointIndex)
                //  .classed("hover", isHoverOver);
    ...
    chart._options = Object.create({}, {
        ...
        xScale:       {get: function(){return x;}, set: function(_){x=_;}},
        yScale: { get: function () { return y; }, set: function (_) { y = _; } },
        y2Scale: {get: function() {return y2;}, set: function(_) {y2=_;}},
        pointScale:   {get: function(){return z;}, set: function(_){z=_;}},
        xDomain:      {get: function(){return xDomain;}, set: function(_){xDomain=_;}},
        yDomain: { get: function () { return yDomain; }, set: function (_) { yDomain = _; } },
        y2Domain: { get: function () { return y2Domain; }, set: function (_) { y2Domain = _; } },
        ...
        x:     {get: function(){return getX;}, set: function(_){getX = d3.functor(_);}},
        y: { get: function () { return getY; }, set: function (_) { getY = d3.functor(_); } },
        y2: {get:function() {return getY2; },set function(_) {getY2 = d3.functor(_);}},
        pointSize: {get: function(){return getSize;}, set: function(_){getSize = d3.functor(_);}},
        pointShape: {get: function(){return getShape;}, set: function(_){getShape = d3.functor(_);}},
        ...
        margin2: {get: function() {return margin2}, set: function(_) {
            margin2.top    = _.top    !== undefined ? _.top    : margin2.top;
            margin2.right  = _.right  !== undefined ? _.right  : margin2.right;
            margin2.bottom = _.bottom !== undefined ? _.bottom : margin2.bottom;
            margin2.left   = _.left   !== undefined ? _.left   : margin2.left;
        ...
        color2: {get: function() {return color2;}, set:function(_) {
            color2 = nv.utils.getColor2(_);
        }},
        useVoronoi: {get: function(){return useVoronoi;}, set: function(_){
            useVoronoi = _;
            if (useVoronoi === false) {
...
//Scatter chart with 2 y-axes
nv.models.scatter2YChart = function() {
    "use strict";
    ...
    var scatter      = nv.models.scatter2Y()
        , xAxis = nv.models.axis()
        , x2Axis = nv.models.axis()
        , yAxis = nv.models.axis()
        , y2Axis = nv.models.axis()
        ...
        , distX        = nv.models.distribution()
        , distY = nv.models.distribution()
        , distY2 = nv.models.distribution2()
        , tooltip = nv.models.tooltip()
    ;
    ...
    scatter.xScale(x).yScale(y).y2Scale(y2);
    xAxis.orient('bottom').tickPadding(10);
    yAxis
        .orient((rightAlignYAxis) ? 'right' : 'left')
        .tickPadding(10)
    ...
    function chart(selection) {
        ...
        if (showXAxis) renderWatch.models(xAxis);
        if (showYAxis) renderWatch.models(yAxis);
        if (showY2Axis) renderWatch.models(y2Axis);
        if (showDistX) renderWatch.models(distX);
        if (showDistY) renderWatch.models(distY);
        if (showDistY2) renderWatch.models(distY2);
        ...
        selection.each(function(data) {
            ...
            nv.utils.initSVG(container);
            var totMargin = { top: Math.max(margin.top, margin2.top), bottom: Math.max(margin.bottom, margin2.bottom), left: Math.max(margin.left, margin2.left), right: Math.max(margin.right, margin.left) }
            var availableWidth = nv.utils.availableWidth(width, container, totMargin),
                availableHeight = nv.utils.availableHeight(height, container, totMargin);
            ...
            if (showY2Axis) {
                gEnter.append('g').attr('class', 'nv-y2 nv-axis').attr("transform", "translate(" + availableWidth + ",0)");
            };
            ...
            scatter
                ...
                .color2(data.map(function(d,i) {
                    d.color2 = d.color2 || color2(d, i);
                    return d.color2;
                }).filter(function (d, i) { return !data[i].disabled }))
                .showLabels(showLabels)
            ...
            regLine.filter(function(d) {
                ...
                .attr('y2', function (d, i) {
                    return y2(x.domain()[0] * d.slope + d.intercept)
                })
                .style('stroke', function (d, i, j) {
                    return color(d, j)
                })
            ...
            if (showY2Axis) {
                ...
                g.select('.nv-y2.nv-axis').call(y2Axis);
            }
            ...
            if (showDistY2) {
                distY2
                    ...
                    .width(availableHeight)
                    .color(data.map(function (d, i) {
                        return d.color2 || color2(d, i);
                    }).filter(function (d, i) { return !data[i].disabled }));
                gEnter.select('.nv-distWrap').append('g')
                    .attr('class', 'nv-distributionY2');
                g.select('.nv-distributionY2')
                    .attr('transform', 'translate(' + (availableWidth) + ',0)')
                    .datum(data.filter(function (d) { return !d.disabled }))
                    .call(distY2);
            }
    ...
    chart._options = Object.create({}, {
        ...
        showDistX:  {get: function(){return showDistX;}, set: function(_){showDistX=_;}},
        showDistY:  {get: function(){return showDistY;}, set: function(_){showDistY=_;}},
        showDistY2:  {get: function(){return showDistY2;}, set: function(_){showDistY2=_;}},
        showLegend: { get: function () { return showLegend; }, set: function (_) { showLegend = _; } },
        showXAxis:  {get: function(){return showXAxis;}, set: function(_){showXAxis=_;}},
        showYAxis: { get: function () { return showYAxis; }, set: function (_) { showYAxis = _; } },
        showY2Axis:  {get: function(){return showY2Axis;}, set: function(_){showY2Axis=_;}},
        ...
        color2: {
            get: function () { return color2; }, set: function (_) {
                color2 = nv.utils.getColor2(_);
                distY2.color(color2);
            }
        }
